...
const paths = [
  ...
  ["topic03_iteration",
    ...
    "assignment17_centered_square.js",
...
export function createTestCase (path2test) {
  ...
  let theFunction = () => {
    ...
    return io.runFileWithInputs(path2test, ...input).then(actualOutputStrings => {
      assert.strictEqual("", actualOutputStrings.pop());
      assert.deepStrictEqual(actualOutputStrings, output);
    });
  };
  ...
  theFunction.andExpectedOutput = (...outputStrings) => {
    output = outputStrings;
    return theFunction;
  };
  theFunction.andExpectedFloatingPoint = (number, fractionDigits) => {
    output = [number.toFixed(fractionDigits)];
    return theFunction;
  };
  ...
  theFunction.andExpectedIntegers = (...numbers) => {
    output = numbers.map(x => x.toString());
    return theFunction;
  };
  theFunction.andExpectedOutputLikeFile = (path2output) => {
    output = readFile(path2output);
    return theFunction;
  };
